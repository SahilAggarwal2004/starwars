...
export default function Modal({ router }) {
	...
	return <>
		...
		<div className={`z-50 w-max max-w-[90vw] max-h-[98vh] overflow-y-auto fixed center text-center bg-white rounded-md py-4 ${type === 'qr-scanner' ? 'px-0' : 'px-4'} ${active ? 'opacity-100' : 'hidden'}`}>
			...
			</div> : type === 'qr-scanner' && <Scanner router={router} />}
		</div>
	</>
}