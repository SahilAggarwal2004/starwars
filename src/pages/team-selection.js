      ...
      {loading ? (
        ...
        <>
          ...
          <div className={`grid fixed x-center bottom-4 gap-x-2.5 min-w-max`} style={{ gridTemplateColumns: `repeat(${players.length}, minmax(0, 1fr))` }}>
            {players.map((player) => (
              <div className="relative size-[6vw] flex justify-center hover:border-2 hover:outline border-transparent rounded-sm" key={player.name} onPointerEnter={() => setHoverPlayer(player)} onPointerLeave={() => setHoverPlayer()} onClick={() => selectPlayer(player)} onContextMenu={(e) => e.preventDefault()}>
